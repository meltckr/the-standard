...
- Edition 004's conversational baseline targeted 190 words per minute after Mel rejected the 150-word-per-minute sample as too slow. The subsequent ending repair preserves that baseline's articulation rate while restoring natural word tails and pauses, so its whole-file average is lower. Do not accelerate the words merely to compensate for restored endings. For new scripts, calculate the tempo from the finalized narration and actual render, then assess the speech and pauses separately. Keep the listener's speed controls available; measurements do not replace listening approval.
- Protect sentence endings during assembly. The Edition 004 repair retains 120 ms after the last signal above -60 dBFS, and 250 ms after the final word. Its 20 ms outgoing fade sits entirely in that quiet tail, not on a final consonant. Do not repeat the old 20 ms tail / 60 ms fade combination. Preserve natural internal pauses instead of deleting them to satisfy a zero-gap test. Inspect unusually short source tails and regenerate a sentence with context when needed; added padding cannot repair a word truncated by generation.
